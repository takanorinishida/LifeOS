...
// Aliased `tr` (not `t`) — classifyType() below already uses a local `const t`
// for the lowercased task text; importing under a different name avoids any
// reader confusion between the two, even though the shadowing is harmless.
import { t as tr } from "../../hooks/lib/work-strings";
...
async function sweepSessions(
  ...
  for (const dir of dirs) {
    ...
    const goalLine = fm.principal_stated_goal
      ? tr(lang, "sweep.session.goalLine", { goal: fm.principal_stated_goal })
      : "";
    const body = [
      tr(lang, "sweep.session.header"),
      goalLine,
      tr(lang, "sweep.session.table", {
        task: taskOrSlug(fm),
        effort: fm.effort,
        mode: fm.mode,
        phase: fm.phase,
        project: fm.project ?? "—",
        slug: fm.slug,
        isaPath: isaPath.replace(HOME, "~"),
        updated: fm.updated,
      }),
      "",
      `---`,
      tr(lang, titlePrefix === "[Native]" ? "sweep.session.footerNative" : "sweep.session.footerAlgorithm"),
    ].filter(Boolean).join("\n");
...
async function sweepProjectChecks(
  ...
  for (const row of rows) {
    ...
    if (age < 14) continue;
    // The "no commits in N days" tail is localized; the "[Project-Check] <name> —"
    // head is NOT (see prefix/dedup note below), so the title is built in two parts.
    const title = `[Project-Check] ${row.name} — ${tr(lang, "sweep.projectCheck.titleSuffix", { age })}`;
    ...
    // This prefix must stay English-literal regardless of locale — it's the
    // dedup key, matched against titles this same function wrote on prior runs.
    ...
    const body = [
      tr(lang, "sweep.projectCheck.body", { project: row.name, path: row.pathLocal.replace(HOME, "~"), age }),
      "",
      `---`,
      tr(lang, "sweep.footer"),
    ].join("\n");
...
async function sweepBpeCadence(
  ...
  const overdue = days === null
    ? tr(lang, "sweep.bpe.titleNever")
    : tr(lang, "sweep.bpe.titleOverdue", { days, cadenceDays: BPE_CADENCE_DAYS });
  ...
  const body = [
    tr(lang, "sweep.bpe.body", { targets: BPE_AUDIT_TARGETS, cadenceDays: BPE_CADENCE_DAYS, lastAudit }),
    "",
    `---`,
    tr(lang, "sweep.footer"),
  ].join("\n");
...
async function main(): Promise<void> {
  ...
  await sweepSessions(cfg.repo, sinceMs, existingLabels, cfg.projectProperty, dryRun, maxCreate, stats, cfg.issueLanguage);
  await sweepStaleIssues(cfg.repo, openIssues, dryRun, stats);
  await sweepProjectChecks(cfg.repo, openIssues, existingLabels, cfg.projectProperty, dryRun, stats, cfg.issueLanguage);
  await sweepGoals(cfg.repo, openIssues, existingLabels, dryRun, stats, cfg.issueLanguage);
  await sweepBpeCadence(cfg.repo, openIssues, existingLabels, dryRun, stats, cfg.issueLanguage);
